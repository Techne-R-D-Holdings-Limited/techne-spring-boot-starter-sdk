 ...
 * 使用方式：
 * <pre>{@code
 * MDC.put(TechneSqlDenyTurboFilter.MDC_KEY, "1");
 * try {
 *     // 执行业务逻辑，SQL 日志自动屏蔽
 * } finally {
 *     MDC.remove(TechneSqlDenyTurboFilter.MDC_KEY);
 * }
 * }</pre>
 * <p>
 * 通过 {@code techne.sql-log-filter.enabled=true} 开启，并可通过
 * {@code techne.sql-log-filter.extra-packages} 配置额外需要屏蔽的包前缀。
 *
 * @author Lil' Doe
 */
public class TechneSqlDenyTurboFilter extends TurboFilter {

    /**
     * MDC 标记键名，值为 "1" 时激活 SQL 日志屏蔽
     */
    public static final String MDC_KEY = "techne_sql_deny";
     ...
     * @author Lil' Doe
     */
    public TechneSqlDenyTurboFilter(List<String> extraPackages) {
