...
import com.techne.boot.exception.TechneException;
 ...
 */
public interface TechneMapper<T> extends BaseMapper<T> {
     ...
     * @return TechnePage
     */
    default <V> TechnePage<V> selectPojoPage(TechnePage<V> page, Wrapper<T> queryWrapper) {
        Assert.notNull(page.getResultClass(), "error: can not execute. because can not find resultClass of page!");
        TechnePage<T> of = TechnePage.of(page.getCurrent(), page.getSize());
    ...
    default boolean insertWithSort(T entity, SFunction<T, Integer> sortColumn, int maxVal) {
        String columnName = TechneUtil.getColumnName(sortColumn);
        String propertyName = TechneUtil.getPropertyName(sortColumn);
        ...
        if (finalVal > maxVal || SqlHelper.retBool(update)) {
            if (val != finalVal) {
                ReflectUtil.setFieldValue(entity, TechneUtil.getPropertyName(sortColumn), finalVal);
            }
    ...
    default boolean updateColumnById(Serializable id, SFunction<T, ?> column, Serializable val) {
        ...
        tableInfo.setPropertyValue(instance, TechneUtil.getPropertyName(column), val);
    ...
    default Class<T> getEntityClass() {
        return (Class<T>) ReflectionKit.getSuperClassGenericType(this.getClass(), TechneMapper.class, 0);
    }
    ...
    default <E extends BasicPage> QueryWrapper<T> buildQueryWrapper(E basicPage, List<SFunction<T, ?>> columns) {
        ...
        String createTime = TechneUtil.getColumnName(AbstractBasicEntity::getCreateTime);
        queryWrapper.ge(ObjUtil.isNotNull(startTime), createTime, startTime)
                    .le(ObjUtil.isNotNull(endTime), createTime, endTime);
        String sql = "CONCAT_WS(' '," + columns.stream().map(TechneUtil::getColumnName).collect(Collectors.joining(StrUtil.COMMA)) + ") like '%" + keyword + "%'";
