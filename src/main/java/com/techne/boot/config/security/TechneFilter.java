package com.techne.boot.config.security;
...
import com.techne.boot.component.TechneAsyncComponent;
 ...
 */
@Slf4j
@AutoConfiguration(value = "techneFilter")
@ConditionalOnMissingBean(name = "techneFilter")
@RequiredArgsConstructor
public class TechneFilter extends OncePerRequestFilter {

    private final TechneAsyncComponent techneAsyncComponent;
    ...
    @Value("${techne.enable-response-data-log:true}")
    ...
    public void initFilterBean() throws ServletException {
        excludeUrlList = new ArrayList<>(List.of(TechneConstants.EXCLUDE_URL));
        if (ArrayUtil.isNotEmpty(techneProperties.getExcludeUrl())) {
            excludeUrlList.addAll(List.of(techneProperties.getExcludeUrl()));
        }
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (excludeUrlList.stream().noneMatch(item -> antPathMatcher.match(item, servletPath))) {
            ...
            String clientIp = TechneUtil.getClientIp(request);
            ...
            log.info("TechneFilter.doFilter --> Request Start: {}", GsonUtil.toJson(map));
            ...
            TechneLog techneLog = (TechneLog) request.getAttribute(RequestConstants.TECHNE_LOG);
            if (ObjUtil.isNotNull(techneLog)) {
                ...
                techneAsyncComponent.insertSysLog(techneLog, loginId, clientIp, userAgent, headerMap, paramObjectValue, request.getMethod(), methodName, request.getRequestURL().toString(), startTime, totalTimeMillis, responseData);
            }
            return;
        }
