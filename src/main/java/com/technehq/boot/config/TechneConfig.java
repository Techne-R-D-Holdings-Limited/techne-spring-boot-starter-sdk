...
import com.technehq.boot.config.satoken.TechneInterceptor;
import com.technehq.boot.config.satoken.TechneSaTokenConfig;
 ...
 */
@AutoConfiguration(value = "techneConfig")
@AutoConfigureOrder(Integer.MIN_VALUE)
@EnableCaching
@EnableRetry
@EnableScheduling
@RequiredArgsConstructor
public class TechneConfig {
    ...
    public MessageSource messageSource(@Value("${spring.messages.basename:}") String basename) {
        ...
        messageSource.addBasenames("techne-i18n/messages");
     ...
     *
     * @return TechneSaTokenConfig
     */
    @Bean
    @ConditionalOnMissingBean
    public TechneSaTokenConfig techneSaTokenConfig() {
        return TechneInterceptor::newInstance;
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public CacheManager cacheManager(RedissonClient redissonClient, @Value("${techne.redisson-cache-config-location:#{null}}") String configLocation) {
