...
import com.technehq.boot.annotation.ApiGroup;
import com.technehq.boot.annotation.ApiSupport;
...
public class OpenApiConfig {
    ...
    public static class CustomModelConverter implements ModelConverter {
        ...
        public Schema<?> resolve(AnnotatedType type, ModelConverterContext context, Iterator<ModelConverter> chain) {
            ...
            if (clazz == Long.class || clazz == Double.class || clazz == Float.class || clazz == BigDecimal.class || clazz == BigInteger.class) {
                ...
                Schema<?> schema = chain.next().resolve(type, context, chain);
                // 使用新的Schema替换原来的，否则example不生效
                return new StringSchema().description(schema.getDescription()).example(LocalDateTime.now().format(TechneDatePattern.UTC_SIMPLE_FORMATTER));
    ...
    public GlobalOperationCustomizer globalOperationCustomizer() {
        ...
        classSet.addAll(ClassUtil.scanPackageBySuper(packageName, TechneCode.class));
        // 全局通用响应信息列表
    ...
    public static class Knife4jOperationIdConfig {
         ...
         */
        @Bean
        public OperationCustomizer techneOperationIdCustomizer() {
            return (operation, handlerMethod) -> {
    ...
    public OpenAPI customOpenApi() {
        String tokenName = saTokenConfig.getTokenName();
        Info info = new Info()
                .description(ResourceUtil.readUtf8Str("techne-markdown/techne.md"))
                .contact(new Contact().name("techne").email("kzerain.kong@kekaokaifa.com").url("https://github.com/techne/techne-spring-boot-starter"));
