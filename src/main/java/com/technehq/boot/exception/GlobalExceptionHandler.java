package com.technehq.boot.exception;
...
import com.technehq.boot.config.StaticConfig;
...
public class GlobalExceptionHandler {
    ...
    public ResponseData<?> exceptionHandler(Exception exception) {
        ...
        if (null != exception.getCause() && exception.getCause().toString().startsWith(SQL_CAUSE)) {
            return ResponseData.retData(TechneCode.DB_ERROR);
        } else {
    ...
    public ResponseData<?> nullPointerException(NullPointerException nullPointerException) {
        ...
        return ResponseData.retData(TechneCode.SYS_NULL_POINT);
    }
    ...
    public ResponseData<?> redisExceptionHandler(RedisException redisException) {
        ...
        return ResponseData.retData(TechneCode.REDIS_ERROR);
    }
    ...
    public ResponseData<?> noSuchElementException(NoSuchElementException noSuchElementException) {
        ...
        return ResponseData.retData(TechneCode.RESOURCE_DOES_NOT_EXIST);
    }
    ...
    public ResponseData<?> illegalArgumentException(IllegalArgumentException illegalArgumentException) {
        ...
        if (StrUtil.startWith(illegalArgumentException.getMessage(), "[Assertion failed]")) {
            // 默认的非法参数异常
            return ResponseData.retData(TechneCode.PARAMETER_ERROR);
        }
        // 自定义的非法参数异常
    ...
    public ResponseData<?> invalidFormatException(InvalidFormatException invalidFormatException) {
        ...
        if (invalidFormatException.getTargetType().isEnum()) {
            ...
            return ResponseData.retData(TechneCode.INVALID_VALUE, new Object[]{invalidFormatException.getValue(), Arrays.toString(enumConstants)});
        }
        return ResponseData.fail(invalidFormatException.getMessage());
    }

    /**
     * 自定义的TechneException异常处理
     *
     * @param techneException 异常
     * @return {@link ResponseData}
     */
    @ExceptionHandler(TechneException.class)
    public ResponseData<?> techneException(TechneException techneException) {
        log.error("GlobalExceptionHandler.techneException --> TechneException: ", techneException);
        return techneException.getResponseData();
    }
    ...
    public ResponseData<?> completionException(CompletionException completionException) {
        ...
        if (rootCause instanceof TechneException techneException) {
            return this.techneException(techneException);
        }
    ...
    public ResponseData<?> parameterBindHandler(BindException bindException) {
        ...
        String msg = TechneCode.PARAMETER_ERROR.getMessage();
        if (ObjUtil.isNotNull(fieldError)) {
            msg = StaticConfig.techneProperties.isIncludeErrorFieldName()
                    ? StrUtil.format("[{}] {}", fieldError.getField(), fieldError.getDefaultMessage())
                    : fieldError.getDefaultMessage();
        }
        return ResponseData.retData(TechneCode.PARAMETER_ERROR.getCode(), msg);
    }
    ...
    public ResponseData<?> notLoginExceptionHandler(NotLoginException notLoginException) {
        ...
        if (notLoginException.getType().equals(NotLoginException.NOT_TOKEN)) {
            // 未提供token
            return ResponseData.retData(TechneCode.NOT_TOKEN);
        } else if (notLoginException.getType().equals(NotLoginException.INVALID_TOKEN)) {
            // token无效
            return ResponseData.retData(TechneCode.INVALID_TOKEN);
        } else if (notLoginException.getType().equals(NotLoginException.TOKEN_TIMEOUT)) {
            // token已过期
            return ResponseData.retData(TechneCode.TOKEN_TIMEOUT);
        } else if (notLoginException.getType().equals(NotLoginException.BE_REPLACED)) {
            // token已被顶下线
            return ResponseData.retData(TechneCode.BE_REPLACED);
        } else if (notLoginException.getType().equals(NotLoginException.KICK_OUT)) {
            // token已被踢下线
            return ResponseData.retData(TechneCode.KICK_OUT);
        } else {
            // 当前会话未登录
            return ResponseData.retData(TechneCode.NOT_LOGGED);
        }
    }
    ...
    public ResponseData<?> saSignExceptionHandler(SaSignException saSignException) {
        ...
        return ResponseData.retData(TechneCode.SIGN_ERROR.cloneWithMessage(saSignException.getMessage()));
    }
    ...
    public ResponseData<?> notRoleExceptionHandler(NotRoleException notRoleException) {
        ...
        return ResponseData.retData(TechneCode.NOT_ROLE_EXCEPTION, new Object[]{notRoleException.getRole()});
    }
    ...
    public ResponseData<?> notPermissionExceptionHandler(NotPermissionException notPermissionException) {
        ...
        return ResponseData.retData(TechneCode.NOT_PERMISSION_EXCEPTION, new Object[]{notPermissionException.getPermission()});
    }
    ...
    public ResponseData<?> disableLoginExceptionHandler(DisableServiceException disableServiceException) {
        ...
        return ResponseData.retData(TechneCode.DISABLE_SERVICE_EXCEPTION, new Object[]{disableServiceException.getDisableTime()});
    }
